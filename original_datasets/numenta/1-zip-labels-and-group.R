...
zip_dataset <- function(groups) {
  for (grp in groups) {
    files_sr <- list.files(path = file.path("original", grp), pattern = "\\.csv$", full.names = TRUE)
    for (series_file in files_sr) {
      series <- read_csv(series_file, show_col_types = FALSE)
      rel <- str_sub(series_file, start = 10, end = -5)
      out_file <- file.path("zip", paste0(rel, ".RData"))
...
for (grp in lab_path) {
  files_lb <- list.files(path = file.path("labels", grp), pattern = "\\.Rds$", full.names = TRUE)
  files_zip <- list.files(path = file.path("zip", grp), pattern = "\\.RData$", full.names = TRUE)
  ...
  for (i in seq_along(files_lb)) {
    ...
    if (nrow(lb) == nrow(series)) {
      ...
      series[1] <- NULL
      out_file <- file.path("ungrouped", grp, basename(files_zip[i]))
      dir.create(dirname(out_file), recursive = TRUE, showWarnings = FALSE)
      save(series, file = out_file, compress = TRUE)
    } else {
...
create_grouped <- function(groups, names_vec) {
  ...
  for (k in seq_along(groups)) {
    files_sr <- list.files(path = file.path("ungrouped", groups[k]), pattern = "\\.RData$")
    group <- list()
    for (i in seq_along(files_sr)) {
      series_file <- file.path("ungrouped", groups[k], files_sr[i])
...
dir.create("grouped", recursive = TRUE, showWarnings = FALSE)
for (j in seq_along(series_path)) {
  numenta_grp <- create_grouped(series_path[j], series_description[j])
  out_file <- file.path("grouped", paste0("numenta_", series_description[j], ".RData"))
...
save(numenta_grp_all, file = "grouped/numenta_grp_all.RData", compress = TRUE)
